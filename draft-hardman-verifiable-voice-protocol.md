...
1. Extract the `orig` field and compare it to the TNAlloc {{#tnalloc}} credential in the dossier to confirm that the AP (or, if OP is not equal to AP and OP is using their own number, the OP) has the right to call on this number.
1. If the passport includes non-null values for the optional `card` or `goal` fields, extract that information and check that the brand attributes claimed for the call are justified by the brand credential in the dossier.
